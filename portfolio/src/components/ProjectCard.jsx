...
export default function ProjectCard({ title, description, tech, demoLink, repoLink, variant, imageUrl, index }) {
  ...
  return (
    <motion.article
      className="relative rounded-2xl overflow-hidden border border-[#D89A8F]/15 bg-[#04070D]/60 flex flex-col group cursor-pointer backdrop-blur-md"
      variants={{ hidden: { opacity: 0, y: 28 }, show: { opacity: 1, y: 0 } }}
      transition={{ duration: 0.72, ease: "easeOut" }}
      whileHover={!isRevealed ? { y: -4, borderColor: "rgba(232, 180, 170, 0.5)" } : { y: -6, borderColor: "rgba(232, 180, 170, 0.5)" }}
    ...
    >
      ...
      >
        <div className="w-full aspect-video overflow-hidden bg-[#05080E] shrink-0">
          <motion.div
        ...
        <div className="flex flex-col flex-1 p-4">
          ...
          <motion.p variants={{ hidden: { opacity: 0, y: 10 }, visible: { opacity: 1, y: 0 } }} className="mt-1 text-xs leading-5 text-[#AFAFAF]">
            {description}
          </motion.p>
      ...
      <AnimatePresence>
        {!isRevealed && (
          <motion.div
            className="absolute inset-0 z-10 flex flex-col justify-between p-6 bg-[#05080E]/60 backdrop-blur-md"
            initial={{ opacity: 1 }}
            exit={{ opacity: 0, backdropFilter: "blur(0px)" }}
            transition={{ duration: 0.5, ease: "easeOut" }}
          >
            {/* Subtle red glow in the corner */}
            <div className="absolute -top-10 -right-10 w-48 h-48 bg-[#D89A8F]/25 rounded-full blur-[50px] pointer-events-none"></div>

            {/* Top Bar */}
            <div className="relative flex justify-between items-center text-[#D89A8F] font-mono text-[11px] font-bold tracking-widest z-20">
            ...
            <div className="relative flex-1 flex flex-col items-center justify-center z-20 mt-4">
              <div className="text-[#D89A8F] text-[9.5px] font-black tracking-[0.25em] uppercase mb-2">Project</div>
            ...
            <div className="relative mt-auto pt-4 z-20">
              <div className="w-full max-w-[180px] px-5 py-2.5 rounded-xl border border-[#D89A8F]/40 bg-[#05080E]/80 backdrop-blur-md text-[#D89A8F] text-[12px] font-bold flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(232, 180, 170, 0.15)] transition-all group-hover:border-[#D89A8F]/80 group-hover:bg-[#D89A8F]/10 mx-auto">
                Click to reveal
